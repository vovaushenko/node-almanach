...
describe('app', () => {
	...
	context('POST /user', () => {
		...
		it('should call user.createStub', (done) => {
	...
	context('DELETE /user/:id', () => {
		...
		it('should call auth check and users.delete on success', (done) => {
	...
	context('handleError', () => {
		...
		it('should check error instance and format error message', (done) => {
		...
		it('should return object as is if it is not an instance of error', (done) => {
			let result = handleError(res, { id: 'foo', message: 'bar' });

			expect(statusStub).to.have.been.calledWith(400);
			expect(jsonStub).to.have.been.calledWith({ id: 'foo', message: 'bar' });
			done();
		});
	});
});
